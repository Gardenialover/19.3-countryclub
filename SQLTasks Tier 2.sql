...
SELECT sub.name AS courtname, CONCAT(m.firstname, ' ', m.surname) AS fullname
FROM (SELECT DISTINCT b.memid,b.facid,f.name
      FROM Bookings AS b
      LEFT JOIN Facilities AS f USING (facid)
      WHERE b.facid IN (0,1)) AS sub
...
FROM 
   (SELECT f.name AS facility, CONCAT(m.firstname,' ', m.surname) AS name,
    ...
    INNER JOIN Facilities AS f ON b.facid = f.facid
    AND b.starttime LIKE "2012-09-14%" 
    INNER JOIN Members AS m ON b.memid = m.memid) sub
WHERE sub.cost>30
...
FROM (
    SELECT facid,memid, EXTRACT(MONTH FROM starttime) AS month
    FROM Bookings 
    WHERE memid !=0
    ) AS s
